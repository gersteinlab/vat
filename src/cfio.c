...
int cfio_get_data (int pid, int filemask)
{
    ...
    if (strcmp (util_getConfigValue ("AWS_USE_S3"), "true") == 0) {
        ...
        if (filemask & OUT_RAW) {
            stringPrintf (src, "%d/vat.%d.raw.vcf", pid, pid);
            stringPrintf (dst, "%s/%d/vat.%d.raw.vcf",
        ...
        if (filemask & OUT_SVMAPPER) {
            stringClear (src);
            stringClear (dst);
            stringPrintf (src, "%d/vat.%d.vcf", pid, pid);
            stringPrintf (dst, "%s/%d/vat.%d.vcf",
                          util_getConfigValue ("WEB_DATA_WORKING_DIR"), pid, pid);
        ...
        if (filemask & OUT_BGZIP) {
            stringClear (src);
            stringClear (dst);
            stringPrintf (src, "%d/vat.%d.vcf.gz", pid, pid);
            stringPrintf (dst, "%s/%d/vat.%d.vcf.gz",
                          util_getConfigValue ("WEB_DATA_WORKING_DIR"), pid, pid);
        ...
        if (filemask & OUT_TABIX) {
            stringClear (src);
            stringClear (dst);
            stringPrintf (src, "%d/vat.%d.vcf.gz.tbi", pid, pid);
            stringPrintf (dst, "%s/%d/vat.%d.vcf.gz.tbi",
                          util_getConfigValue ("WEB_DATA_WORKING_DIR"), pid, pid);
        ...
        if (filemask & OUT_GENE_SUMMARY) {
            stringClear (src);
            stringClear (dst);
            stringPrintf (src, "%d/vat.%d.geneSummary.txt", pid, pid);
            stringPrintf (dst, "%s/%d/vat.%d.geneSummary.txt",
                          util_getConfigValue ("WEB_DATA_WORKING_DIR"), pid, pid);
        ...
        if (filemask & OUT_SAMPLE_SUMMARY) {
            stringClear (src);
            stringClear (dst);
            stringPrintf (src, "%d/vat.%d.sampleSummary.txt", pid, pid);
            stringPrintf (dst, "%s/%d/vat.%d.sampleSummary.txt",
                          util_getConfigValue ("WEB_DATA_WORKING_DIR"), pid, pid);
        ...
        if (filemask & OUT_RAW) {
            stringPrintf (src, "%s/%d/vat.%d.raw.vcf",
                          util_getConfigValue ("WEB_DATA_DIR"), pid, pid);
            stringPrintf (dst, "%s/%d/vat.%d.raw.vcf",
                          util_getConfigValue ("WEB_DATA_WORKING_DIR"), pid, pid);
        ...
        if (filemask & OUT_SVMAPPER) {
            stringClear (src);
            stringClear (dst);
            stringPrintf (src, "%s/%d/vat.%d.vcf",
                          util_getConfigValue ("WEB_DATA_DIR"), pid, pid);
            stringPrintf (dst, "%s/%d/vat.%d.vcf",
                          util_getConfigValue ("WEB_DATA_WORKING_DIR"), pid, pid);
        ...
        if (filemask & OUT_BGZIP) {
            stringClear (src);
            stringClear (dst);
            stringPrintf (src, "%s/%d/vat.%d.vcf.gz",
                          util_getConfigValue ("WEB_DATA_DIR"), pid, pid);
            stringPrintf (dst, "%s/%d/vat.%d.vcf.gz",
                          util_getConfigValue ("WEB_DATA_WORKING_DIR"), pid, pid);
        ...
        if (filemask & OUT_TABIX) {
            stringClear (src);
            stringClear (dst);
            stringPrintf (src, "%s/%d/vat.%d.vcf.gz.tbi",
                          util_getConfigValue ("WEB_DATA_DIR"), pid, pid);
            stringPrintf (dst, "%s/%d/vat.%d.vcf.gz.tbi",
                          util_getConfigValue ("WEB_DATA_WORKING_DIR"), pid, pid);
        ...
        if (filemask & OUT_GENE_SUMMARY) {
            stringClear (src);
            stringClear (dst);
            stringPrintf (src, "%s/%d/vat.%d.geneSummary.txt",
                          util_getConfigValue ("WEB_DATA_DIR"), pid, pid);
            stringPrintf (dst, "%s/%d/vat.%d.geneSummary.txt",
                          util_getConfigValue ("WEB_DATA_WORKING_DIR"), pid, pid);
        ...
        if (filemask & OUT_SAMPLE_SUMMARY) {
            stringClear (src);
            stringClear (dst);
            stringPrintf (src, "%s/%d/vat.%d.sampleSummary.txt",
                          util_getConfigValue ("WEB_DATA_DIR"), pid, pid);
            stringPrintf (dst, "%s/%d/vat.%d.sampleSummary.txt",
                          util_getConfigValue ("WEB_DATA_WORKING_DIR"), pid, pid);
...
int cfio_push_data (int pid)
{
    ...
    if (strcmp (util_getConfigValue ("AWS_USE_S3"), "true") == 0) {
        ...
        stringPrintf (src, "%s/%d/vat.%d.raw.vcf",
                      util_getConfigValue ("WEB_DATA_WORKING_DIR"), pid, pid);
        stringPrintf (dst, "%d/vat.%d.raw.vcf", pid, pid);
        ...
        stringPrintf (src, "%s/%d/vat.%d.vcf",
                      util_getConfigValue ("WEB_DATA_WORKING_DIR"), pid, pid);
        stringPrintf (dst, "%d/vat.%d.vcf", pid, pid);
        if (s3_put (string (src), bucket, string (dst)) != 0) {
            fprintf (stderr, "Could not put %s to bucket %s",
                     string (src), bucket);
            return -1;
        ...
        stringPrintf (src, "%s/%d/vat.%d.vcf.gz",
                      util_getConfigValue ("WEB_DATA_WORKING_DIR"), pid, pid);
        stringPrintf (dst, "%d/vat.%d.vcf.gz", pid, pid);
        if (s3_put (string (src), bucket, string (dst)) != 0) {
            fprintf (stderr, "Could not put %s to bucket %s",
                     string (src), bucket);
            return -1;
        ...
        stringPrintf (src, "%s/%d/vat.%d.vcf.gz.tbi",
                      util_getConfigValue ("WEB_DATA_WORKING_DIR"), pid, pid);
        stringPrintf (dst, "%d/vat.%d.vcf.gz.tbi", pid, pid);
        if (s3_put (string (src), bucket, string (dst)) != 0) {
            fprintf (stderr, "Could not put %s to bucket %s",
                     string (src), bucket);
            return -1;
        ...
        stringPrintf (src, "%s/%d/vat.%d.sampleSummary.txt",
                      util_getConfigValue ("WEB_DATA_WORKING_DIR"), pid, pid);
        stringPrintf (dst, "%d/vat.%d.sampleSummary.txt", pid, pid);
        if (s3_put (string (src), bucket, string (dst)) != 0) {
            fprintf (stderr, "Could not put %s to bucket %s",
                     string (src), bucket);
            return -1;
        ...
        stringPrintf (src, "%s/%d/vat.%d.geneSummary.txt",
                      util_getConfigValue ("WEB_DATA_WORKING_DIR"), pid, pid);
        stringPrintf (dst, "%d/vat.%d.geneSummary.txt", pid, pid);
        if (s3_put (string (src), bucket, string (dst)) != 0) {
            fprintf (stderr, "Could not put %s to bucket %s",
                     string (src), bucket);
            return -1;
        ...
        stringClear (src);
        stringClear (dst);
        stringPrintf (src, "%s/%d/vat.%d",
                      util_getConfigValue ("WEB_DATA_WORKING_DIR"), pid, pid);
        stringPrintf (dst, "%d/vat.%d", pid, pid);
        if (s3_put_dir (string (src), bucket, string (dst)) != 0) {
...
int cfio_clear_working (int pid)
{
    ...
    stringPrintf (file, "%s/%d",
                  util_getConfigValue ("WEB_DATA_WORKING_DIR"), pid);
    if (shell_rmrf (string (file)) != 0) {
